...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        cv2.imshow('ROI Setup - Camera 3', frame)
...
def setup_roi(video_path):
    ...
    # Window setup
    cv2.namedWindow('ROI Setup - Camera 3')
    cv2.setMouseCallback('ROI Setup - Camera 3', mouse_callback)
    
    # Print instructions
    print("\n" + "="*60)
    print("        Seat ROI Setup Program (Camera 3)")
    print("="*60)
    print("\nHow to use:")
    ...
    while True:
        ...
        if key == ord('s'):
            if len(seats) > 0:
                with open('seat_roi_cam3.json', 'w') as f:
                    json.dump(seats, f, indent=2)
                print(f"\n✅ {len(seats)} seat(s) saved to 'seat_roi_cam3.json'!")
