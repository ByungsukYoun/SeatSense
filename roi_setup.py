...
def setup_roi(video_path):
    ...
    while True:
        ...
        if key == ord('s'):
            if len(seats) > 0:
                with open('seat_roi.json', 'w') as f:
                    json.dump(seats, f, indent=2)
                print(f"\n{len(seats)} seat(s) saved to 'seat_roi.json'!")
            ...
            if len(seats) > 0:
                ...
                for seat in seats:
                    ...
                    cv2.putText(frame, f'Seat {seat["id"]}', (center_x-30, center_y), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                cv2.imshow('ROI Setup', frame)
                print(f"Seat {removed_seat['id']} removed (remaining seats: {len(seats)})")
            else:
